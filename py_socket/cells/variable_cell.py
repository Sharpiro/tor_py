...
class VariableCell:
    ...
    HEADER_SIZE = CIRCUIT_ID_SIZE + COMMAND_SIZE + PAYLOAD_LENGTH_SIZE
    PACK_FORMAT = "HBH" if CIRCUIT_ID_SIZE == 2 else "LBH"
...
def unpack_variable_cell(buffer: bytes) -> (VariableCell, int):
    ...
    payload = unpack(f">{payload_length}s",
                     buffer[VariableCell.HEADER_SIZE:VariableCell.HEADER_SIZE + payload_length])[0]
...
def pack_variable_cell(variable_cell: VariableCell) -> bytes:
    headers_buffer = pack(f">{variable_cell.PACK_FORMAT}", variable_cell.circuit_id,
                          variable_cell.command.value, variable_cell.payload_length)
