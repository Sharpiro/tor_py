...
def unpack_cell(buffer: bytes) -> (Cell, int):
    circuit_id, command = unpack(f">{Cell.PACK_FORMAT}", buffer[:Cell.HEADER_SIZE])
    payload = unpack(f">{Cell.PAYLOAD_LENGTH}s", buffer[Cell.HEADER_SIZE:Cell.HEADER_SIZE + Cell.PAYLOAD_LENGTH])[0]
